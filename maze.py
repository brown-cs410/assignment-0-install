...
class Maze(SearchProblem[MazeState]):
    ...
    def get_successors(self, state: MazeState) -> Dict[MazeState, int]:
        ...
        position in the maze and adds valid moves to the list of successors. A move is considered 
        valid if there is no wall blocking the path in that direction. The successors are returned 
        as a dictionary where the keys are the resulting MazeState objects and the values are the 
        associated costs (typically 1).

        Args:
            state (MazeState): The current state of the maze.

        Returns:
            Dict[MazeState, int]: A dictionary mapping each valid successor state to the cost of 
