...
class MazeGenerator(SearchProblem[MazeState]):
    ...
    def __init__(self, width: int, height: int):
        ...
        self.visited_cells_count = 1  # Starts with the initial cell visited
    ...
    # TODO: Fill out this method
    def is_goal_state(self, state: MazeState) -> MazeState:
        """
    ...
    def get_successors(self, state: MazeState) -> Dict[MazeState, int]:
        """
        ...
        Returns:
            dict: A dictionary where keys are successor MazeState objects and values 
                are the associated costs of moving to those states (typically 1).
        """
    ...
    def is_in_range(self, row: int, col: int) -> bool:
        """
        Helper Function: Checks whether a given position is within the bounds of the maze.
    ...
    def opposite_direction(self, direction: str) -> str:
        """
        Helper Function: Returns the opposite direction of the given direction.
...
def main():
    ...
    if bfs_path:
        bfs_maze = Maze(generator.height, generator.width, board=bfs_path[-1].board) 
